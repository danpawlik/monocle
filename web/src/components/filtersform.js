...
class FiltersForm extends React.Component {
    ...
    const resume = (
      <b>
        ...
        {params.get("state") && this.props.showChangeParams
          ...
          : ""}
        {params.get("project_definition")
          ? ", change project_definition " + params.get("project_definition")
          : ""}
        {"."}
      </b>
    );
  ...
  handleProjectDefState = (e) => {
    const selected = e.target.attributes.value.value;
    this.setState({ project_definition: selected });
  };

  renderProjectDefs = () => {
    getConfigProjectDefinitions(this.props.location.pathname).then((res) =>
  ...
  render() {
    ...
    const project_defs = this.state.project_defs ? (
      this.state.project_defs.map((response) => (
        <Dropdown.Item
          key={response.name}
          value={response.name}
          active={response.name === this.state.project_def_state}
                        ...
                        <Row>
                          ...
                          <Col md={6}>
                            ...
                            <Form.Group controlId="formProjectDefinitionInput">
                              <DropDownButton
                                title={
                                  this.state.project_definition
                                    ? "Project definition daniel: " +
                                      this.state.project_definition
                                    : "Project definition: "
                                }
                                size="sm"
                              ...
                              >
                                {project_defs}
                                {/*
                                {this.renderProjectDefs().then((res) => {
                                  res.map((response) => (
                                    <Dropdown.Item
                                      key={response.name}
                                      value={response.name}
